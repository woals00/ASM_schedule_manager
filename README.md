...
- 멘토링 시작 `1시간 전` 또는 `30분 전`에 Discord Webhook 또는 Telegram Bot으로 알림을 보낼 수 있습니다.
...
접수 내역 대시보드의 `외부 알림 동기화` 섹션에서 아래 값을 입력합니다.
...
저장 후 `지금 동기화`를 누르면 예정된 신청 일정이 Cloudflare로 업로드됩니다.

내일 일정이 있으면 `테스트 알림` 버튼으로 해당 일정 기준 테스트 메시지를 바로 보내볼 수 있습니다. 내일 일정이 없으면 가장 가까운 다음 일정을 기준으로 테스트합니다.
